...
cd build/l2l3_acl/; make clean; make; cd ../..
# controllers for the examples
declare -A controller_for_example
controller_for_example["l2l3_acl"]="dpdk_controller"
...
controller_for_example["ndn_routing_test"]="dpdk_l3_controller"
controller_for_example["ipv6_routing_test"]="dpdk_l3-full_controller"
...
#sleep 1
#perf stat -d -p `pidof l2_switch_test` &
#3>output.log perf stat -e instructions,L1-dcache-load,L1-dcache-loads-misses,L1-dcache-stores,LLC-load,LLC-loads-misses,LLC-store,LLC-stores-misses,task-clock,cycles -p `pidof l2_switch_test` --log-fd 3 &
#sleep 60
#kill -SIGINT `pidof perf`
#kill -9 `pidof l2_switch_test`
#cat output.log
#awk 'BEGIN{ORS="";print l}{print $4;print "\t\t"}'  output.log
#rm output.log
#kill -9 `pidof dpdk_controller`
