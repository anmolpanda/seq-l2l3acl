...
void           exact_add_universal (lookup_table_t* t, struct universal_key key,                uint8_t* value);
...
uint8_t*      one_exact_lookup(lookup_table_t* t, uint8_t* key);
//void          exact_lookup (lookup_table_t* t, int batch_size, int keysize, uint8_t key[][keysize], uint8_t **values); //Commented because it has been modified in dataplane.c
void          exact_lookup (lookup_table_t* t, int batch_size, int keysize, struct universal_key key[], uint8_t **values);
void          bulk_exact_lookup(lookup_table_t* t, int batch_size, int keysize, uint8_t key[][keysize], uint8_t **values);
void          lpm_lookup(lookup_table_t* t, int batch_size, int keysize, uint8_t key[][keysize], uint8_t* value[]);
uint8_t*      ternary_lookup (lookup_table_t* t, uint8_t* key);
//uint8_t*	ternary_lookup(lookup_table_t* t, int batch_size, int keysize, uint8_t key[][keysize], uint8_t* value[]);
//=============================================================================
// Primitive actions
