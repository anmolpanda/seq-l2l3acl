...
def format_state(state):
    ...
    if isinstance(state, p4.p4_parse_state):
        #[    return parse_state_${state.name}(pd, batch_size, tables);
...
for state_name, parse_state in hlir.p4_parse_states.items():
    ...
    for call in parse_state.call_sequence:
		...
		for state_name1, parse_state1 in hlir.p4_parse_states.items():
	            for call1 in parse_state1.call_sequence:
	    			...
	    			#[         buf += header_instance_byte_width[${header_instance_name1}];
            #[         extract_header(buf, &pd[i], ${header_instance_name});
	...
	if call_number > 0:
		...
		#[             total_keys++;
		#[             if(unlikely(total_keys == batch_size))
                #[                 ${format_state(next_state)}         }
...
#[     parse_state_start(pd, batch_size, tables);
#[ }
